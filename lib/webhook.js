...
Webhooks.prototype.ensureRepo = function(repoClient, repo, callback){
  ...
  ghRepo.hooks(function(e, hooks){
    ...
    hooks.every(function(hook){
      ...
      util.log.info('checking hook ' + hook.config.url);

      var eventsMatch = true;

      repo.events.forEach(function(event){
        if (hook.events.indexOf(event) == -1) eventsMatch = false;
      });

      if (hook.config.url == _this.options.url && eventsMatch) {
        found = true;
        util.log.info('matching hook found: ' + hook.config.url);
      }
      return found;
    });
